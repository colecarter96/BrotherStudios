    ...
    <>
        <div className="px-4 md:px-8">
          ...
          {productsNormalized.map((p) => {
            const baseImg = p.variants[0]?.images?.[1] || p.variants[0]?.images?.[0];
            // If a second variant exists, prefer its second image on hover; fallback to its first
            const altVariantSecond = p.variants[1]?.images?.[1];
            const altVariantFirst = p.variants[1]?.images?.[0];
            const sameVariantFirst = p.variants[0]?.images?.[0];
            const hoverImg = altVariantSecond || altVariantFirst || (sameVariantFirst && sameVariantFirst !== baseImg ? sameVariantFirst : undefined);
              ...
              >
                <div className="relative w-full aspect-square overflow-hidden">
                  ...
                  <div className="absolute -inset-[2px]">
                    <Image
                      src={baseImg || p.variants[0]?.images?.[0]}
                      alt={p.title}
                      fill
